...
# %% auto 0
__all__ = ['d_model', 'd_ff', 'Head', 'MultiHeadAttention', 'FeedForward', 'Block', 'GPT2']
...
class Head(nn.Module):
    def __init__(self, content_lenght: int, n_embd: int, head_size: int):
        super().__init__()
        self.key = nn.Linear(n_embd, head_size, bias=False)
        self.query = nn.Linear(n_embd, head_size, bias=False)
        self.value = nn.Linear(n_embd, head_size, bias=False)
        self.register_buffer("tril", torch.tril(torch.ones(content_lenght, content_lenght)))
...
class MultiHeadAttention(nn.Module):
    def __init__(self, n_heads, head_size, n_embd) -> None:
        super().__init__()
...
class Block(nn.Module):
    def __init__(self, n_embd: int, n_head: int) -> None:
        ...
        self.attn = MultiHeadAttention(n_head, head_size, n_embd)
...
class GPT2(nn.Module):
    def __init__(self, vocab_size: int, n_embd: int, context_length: int):
        """_summary_
        ...
        self.position_embedding = nn.Embedding(vocab_size, n_embd)
    ...
    def forward(self, idx, targets=None):
        ...
        x = self.attn_heads(x) # (BATCH_SIZE, SEQ_LEN, n_embd)
